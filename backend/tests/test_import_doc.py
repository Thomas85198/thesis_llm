...
def test_markdown_marks_and_inline_math():
    ...
    marks = {
        ...
        for n in para["content"] if n["type"] == "text"
    }
    assert ("bold",) in marks and ("italic",) in marks
    assert ("code",) in marks and ("strike",) in marks
    assert any(n["type"] == "mathInline" and n["attrs"]["latex"] == "E=mc^2"
...
def test_latex_sections_and_marks():
    ...
    bolds = [n for n in _find(doc, "text") if any(m["type"] == "bold" for m in n.get("marks", []))]
    assert any(n["text"] == "bold" for n in bolds)
...
def test_markdown_roundtrip_preserves_blocks():
    ...
    md = export_doc.to_markdown({"title": "我的論文", "content_json": original}, "apa", "參考文獻")
...
def test_demote_pseudo_headings():
    blocks = [
        ...
        {"type": "heading", "attrs": {"level": 3},
         "content": [{"type": "text", "text": "這是一段很長的內文，說明研究背景與動機，並以句號結尾。"}]},
...
def test_relink_directories():
    blocks = [
        {"type": "heading", "attrs": {"level": 1}, "content": [{"type": "text", "text": "目錄"}]},
        {"type": "paragraph", "content": [{"type": "text", "text": "第一章\t"}]},  # tab debris
        {"type": "paragraph"},                                                     # empty debris
        {"type": "heading", "attrs": {"level": 1}, "content": [{"type": "text", "text": "圖目錄"}]},
        {"type": "paragraph"},
        {"type": "heading", "attrs": {"level": 1}, "content": [{"type": "text", "text": "表目錄"}]},
        {"type": "paragraph"},
        {"type": "heading", "attrs": {"level": 1}, "content": [{"type": "text", "text": "第一章 緒論"}]},
        {"type": "paragraph", "content": [{"type": "text", "text": "正文開始"}]},
    ]
    out = import_doc._relink_directories(blocks)
    assert [b["type"] for b in out] == [
        "heading", "tableOfContents",   # 目錄 → live TOC, debris dropped
        "heading", "figureList",        # 圖目錄 (matched before 目錄)
        "heading", "tableList",         # 表目錄
        "heading", "paragraph",         # real content untouched
    ]
...
def test_relink_keeps_leader_dot_page_numbers_out():
    blocks = [
        {"type": "heading", "attrs": {"level": 1}, "content": [{"type": "text", "text": "目錄"}]},
        {"type": "paragraph", "content": [{"type": "text", "text": "第一章 緒論……12"}]},  # leader dots + page
        {"type": "paragraph", "content": [{"type": "text", "text": "這是真正的正文段落"}]},
    ]
    out = import_doc._relink_directories(blocks)
...
def test_unwrap_pseudo_blockquotes():
    blocks = [
        {"type": "blockquote", "content": [{"type": "paragraph", "content": [
            {"type": "text", "text": "Bhattacherjee, A. (2001). Understanding IS continuance. MIS Quarterly, 25(3)."}]}]},
        {"type": "blockquote", "content": [{"type": "paragraph", "content": [
            {"type": "text", "text": "知識就是力量"}]}]},  # short pull-quote, no full stop
    ]
...
def test_attach_figure_and_table_captions():
    blocks = [
        ...
        {"type": "paragraph", "content": [{"type": "text", "text": "圖1-1 2018全球社群媒體使用概況"}]},
        {"type": "paragraph", "content": [{"type": "text", "text": "表2-1 社群媒體分類"}]},
        {"type": "tableBlock", "content": [{"type": "tableCaption"}, {"type": "table", "content": []}]},
    ]
    out = import_doc._attach_captions(blocks)
    assert [b["type"] for b in out] == ["figure", "tableBlock"]  # caption paragraphs folded in
    assert out[0]["attrs"]["caption"] == "2018全球社群媒體使用概況"  # 圖 prefix stripped
...
def test_attach_captions_ignores_non_caption_paragraph():
    blocks = [
        {"type": "figure", "attrs": {"src": "/x.png", "caption": "", "alt": ""}},
        {"type": "paragraph", "content": [{"type": "text", "text": "這是正文不是圖說。"}]},
    ]
    out = import_doc._attach_captions(blocks)
    assert len(out) == 2 and out[0]["attrs"]["caption"] == ""  # body paragraph left alone
