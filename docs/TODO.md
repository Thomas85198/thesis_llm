...
- **產品端**：審稿 + AI 寫作編輯器兩大子系統都上線；編輯器已到 **v4.21.0**（三格式一致含數學式 OMML、專注模式、文法 lint、缺陷一鍵套用、五格式匯入含 PDF、論文助手串流+持久化皆完成）。中英 i18n + 深色模式已併入 `main`。
...
- ✅ **DOCX 數學式轉 OMML**（2026-07-14，v4.21.0，使用者已驗收）：latex2mathml→
  mathml2omml→OMML，失敗回退字面文字；連帶補 amsmath、Mac Word 空白修復、
  寬式子捲動。已知限制：aligned 多行環境退回字面。三格式一致北極星收官。
