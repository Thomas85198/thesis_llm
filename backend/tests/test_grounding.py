...
def test_ground_uses_fulltext_and_ranks(monkeypatch):
    ...
    def upsert(oid, source, chunks):
        store[oid] = [{"idx": i, "text": t, "embedding": e, "source": source} for i, t, e in chunks]
    monkeypatch.setattr(db, "upsert_paper_chunks", upsert)
    monkeypatch.setattr(grounding, "_fetch_fulltext",
                        lambda url: "Self-attention models long-range dependencies effectively here. "
...
def test_ground_falls_back_to_abstract(monkeypatch):
    store = {}
    monkeypatch.setattr(db, "get_paper_chunks", lambda oid: store.get(oid, []))
    monkeypatch.setattr(db, "upsert_paper_chunks",
        lambda oid, source, chunks: store.__setitem__(oid, [{"idx": i, "text": t, "embedding": e, "source": source} for i, t, e in chunks]))
    monkeypatch.setattr(grounding, "_fetch_fulltext", lambda url: "")  # no full text
    monkeypatch.setattr(openalex, "get_works_by_ids",
                        lambda ids: {"W1": {"abstract": "We propose a model for long-range dependencies in sequences."}})
...
def test_ground_empty_claim_returns_none(monkeypatch):
    monkeypatch.setattr(llm, "embed", lambda texts: (_ for _ in ()).throw(AssertionError("should not embed")))
    assert grounding.ground("W1", "u", "  ", "d") == {"source": "none", "supporting": []}
